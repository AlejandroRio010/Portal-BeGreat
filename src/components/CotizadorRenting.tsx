...
export default function CotizadorRenting() {
  ...
  function calcular() {
    ...
    setResultados(res);
    if (!seleccionado) setSeleccionado(48);
  }

  const fmt = (n: number) => n.toLocaleString("es-ES", { minimumFractionDigits: 2, maximumFractionDigits: 2 });

  const visibleResultados = resultados
    ? expanded ? resultados : resultados.filter(r => PLAZOS_DEFAULT.includes(r.plazo))
    : null;
    ...
    <div>
      <div className="bg-[#2E1A47]">
        ...
        <div className="p-6 pb-5">
          <p className="text-white/50 text-[10px] font-bold uppercase tracking-[0.2em] mb-5">Importe de factura (neto)</p>
          <div className="flex gap-3">
            <input value={cotImporte} onChange={e => setCotImporte(e.target.value)}
              ...
              className="flex-1 px-4 py-3.5 bg-white text-sm text-gray-900 font-semibold focus:outline-none focus:ring-2 focus:ring-white/40"
              placeholder="10.000,00 €" />
            <button type="button" onClick={calcular}
              className="px-8 py-3.5 bg-white/20 text-white text-sm font-bold uppercase tracking-wider hover:bg-white/30 transition-colors whitespace-nowrap border border-white/30">
              Calcular operación
            </button>
          </div>
        </div>
        ...
        {visibleResultados && (
          <div className="px-6 pb-2">
            <div className="flex border-b border-white/20 pb-2.5 mb-1">
            ...
            <div className="divide-y divide-white/10">
              {visibleResultados.map(r => (
                <label key={r.plazo}
                  className={`flex items-center py-3.5 cursor-pointer transition-all ${seleccionado === r.plazo ? "bg-white/10 -mx-3 px-3" : "hover:bg-white/5 -mx-3 px-3"}`}>
            ...
            {/* Expand / collapse */}
            {resultados && resultados.length > PLAZOS_DEFAULT.length && (
