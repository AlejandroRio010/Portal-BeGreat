...
import { db } from "@/db";
import { officeContactNotes } from "@/db/schema";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ contactId: string }> }) {
  ...
  const [note] = await db.insert(officeContactNotes).values({ contact_id, author_id: session.user!.id as string, author_name, texto: texto.trim() }).returning();
...
export async function DELETE(req: NextRequest) {
  ...
  if (!noteId) return NextResponse.json({ error: "Datos requeridos" }, { status: 400 });
  await db.delete(officeContactNotes).where(eq(officeContactNotes.id, noteId));
  return NextResponse.json({ ok: true });
}
