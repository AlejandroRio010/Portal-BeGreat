...
export async function POST(req: NextRequest, { params }: { params: Promise<{ contactId: string }> }) {
  ...
  const [note] = await db.insert(entityContactNotes).values({ contact_id, author_id: session.user!.id as string, author_name, texto: texto.trim() }).returning();
...
export async function PATCH(req: NextRequest) {
  ...
  const [note] = await db.select().from(entityContactNotes).where(eq(entityContactNotes.id, noteId)).limit(1);
  ...
  await db.update(entityContactNotes).set(updateData).where(eq(entityContactNotes.id, noteId));
...
export async function DELETE(req: NextRequest) {
  ...
  if (!noteId) return NextResponse.json({ error: "Datos requeridos" }, { status: 400 });
  await db.delete(entityContactNotes).where(eq(entityContactNotes.id, noteId));
  return NextResponse.json({ ok: true });
}
