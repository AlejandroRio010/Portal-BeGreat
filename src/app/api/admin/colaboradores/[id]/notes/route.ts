...
import { db } from "@/db";
import { collaboratorNotes } from "@/db/schema";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [note] = await db.insert(collaboratorNotes).values({ collaborator_id, author_id: session.user!.id as string, author_name, texto: texto.trim() }).returning();
...
export async function PATCH(req: NextRequest) {
  ...
  const [note] = await db.select().from(collaboratorNotes).where(eq(collaboratorNotes.id, noteId)).limit(1);
  ...
  await db.update(collaboratorNotes).set(updateData).where(eq(collaboratorNotes.id, noteId));
...
export async function DELETE(req: NextRequest) {
  ...
  if (!noteId) return NextResponse.json({ error: "Datos requeridos" }, { status: 400 });
  await db.delete(collaboratorNotes).where(eq(collaboratorNotes.id, noteId));
  return NextResponse.json({ ok: true });
}
