...
import { db } from "@/db";
import { officeNotes, collaborators } from "@/db/schema";
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [note] = await db.select().from(officeNotes).where(eq(officeNotes.id, id)).limit(1);
  ...
  const [updated] = await db.update(officeNotes).set(updates).where(eq(officeNotes.id, id)).returning();
