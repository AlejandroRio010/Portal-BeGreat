...
import { entityOfficeContacts, officeContactNotes, financialEntities, entityOffices } from "@/db/schema";
...
export default async function AdminOfficeContactoPage({ params }: { params: Promise<{ id: string; contactoId: string }> }) {
  ...
  const [contacto] = await db.select({
    ...
    office_id: entityOfficeContacts.office_id,
