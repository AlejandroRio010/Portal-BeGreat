...
function ContactoRow({ c }: { c: Contacto }) {
  ...
  async function handleDelete() {
    setDeleting(true);
    const res = await fetch(`/api/admin/entidades/oficinas/contactos/${c.id}`, { method: "DELETE" });
...
function NuevoContactoForm({ officeId }: { officeId: string }) {
  const router = useRouter();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/admin/entidades/oficinas/${officeId}/contactos`, {
        method: "POST",
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-3 border border-gray-200 bg-gray-50">
      ...
      <div className="px-4 py-3 space-y-2">
        ...
        {[["Puesto en la empresa", "rol"], ["Email", "email"], ["Teléfono", "telefono"]].map(([lbl, key]) => (
          <div key={key}>
...
export default function ContactosOficinaPanel({ contactos, officeId }: { contactos: Contacto[]; officeId: string }) {
  return (
