...
export default async function ContactoFichaPage({
  ...
  const [contacto] = await db
    ...
    .where(eq(entityOfficeContacts.id, contactoId))
    .limit(1);
  ...
  return (
    <div className="min-h-screen bg-[#f8f7fb]">
      ...
      <div className="px-8 pt-6 pb-2 flex items-center gap-2 text-xs text-gray-500 flex-wrap">
        ...
        <span>/</span>
        <Link href={`/admin/entidades/${entityId}/oficinas/${oficineId}`} className="hover:text-[#2E1A47] font-medium">{oficina?.nombre}</Link>
        <span>/</span>
        ...
        <div className="flex flex-col gap-4">
          <div className="bg-white border border-gray-200">
            ...
            <div className="px-5 py-4 divide-y divide-gray-50">
              {[
                { label: "Puesto en la empresa", value: contacto.rol },
                ...
                { label: "LinkedIn", value: contacto.linkedin, href: contacto.linkedin ?? undefined },
                ...
                <div key={field.label} className="py-2.5 flex flex-col gap-0.5">
                  ...
                  {field.href ? (
                    <a href={field.href} target={field.label === "LinkedIn" ? "_blank" : undefined} rel="noopener noreferrer"
                      className="text-sm text-[#2E1A47] hover:underline font-medium break-all">{field.value}</a>
                  ) : (
          ...
          <div className="bg-white border border-gray-200 p-4">
            ...
            <Link href={`/admin/entidades/${entityId}/oficinas/${oficineId}`} className="text-sm font-semibold text-[#2E1A47] hover:underline">
              {oficina?.nombre} →
            </Link>
            <p className="text-[10px] text-gray-400 uppercase tracking-wider mt-3 mb-1">Entidad</p>
            <Link href={`/admin/entidades/${entityId}`} className="text-sm font-semibold text-[#2E1A47] hover:underline">
              {entidad?.nombre} →
            </Link>
          </div>
        </div>
