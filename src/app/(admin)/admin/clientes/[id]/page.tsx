                  ...
                  {avalistas.map((a, i) => (
                    <div key={i} className="flex items-center justify-between bg-gray-50 border border-gray-100 px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          {a.aval_contact_id ? (
                            <Link href={`/admin/clientes/${id}/contactos/${a.aval_contact_id}`} className="text-sm font-semibold text-[#2E1A47] hover:underline">{a.aval_nombre}</Link>
                          ) : a.aval_client_id ? (
                            <Link href={`/admin/clientes/${a.aval_client_id}`} className="text-sm font-semibold text-[#2E1A47] hover:underline">{a.aval_nombre}</Link>
                          ) : (
                ...
                {ops.filter(o => (opDocsMap.get(o.id) ?? []).length > 0).map(o => (
                  <div key={o.id} className="border border-gray-200">
                    <div className="bg-[#EEEBF3] px-4 py-2.5 flex items-center justify-between">
                      <Link href={`/admin/operaciones/${o.id}`} className="text-xs font-bold text-[#2E1A47] hover:underline">{o.nombre ?? "Sin nombre"}</Link>
