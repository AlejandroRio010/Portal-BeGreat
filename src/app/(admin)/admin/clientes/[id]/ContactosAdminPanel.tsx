...
function ContactoRow({ c }: { c: Contacto }) {
  ...
  const [form, setForm] = useState({ nombre: c.nombre, rol: c.rol ?? "", email: c.email ?? "", telefono: c.telefono ?? "", linkedin: c.linkedin ?? "" });
  ...
  if (editing) {
    return (
      <form onSubmit={handleSave} className="py-3 space-y-2 border-b border-gray-50">
        <div>
          <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Nombre *</label>
          <ContactoAutocomplete value={form.nombre} onChange={v => set("nombre", v)}
            onSelect={p => setForm(f => ({ ...f, nombre: p.nombre, rol: p.rol ?? f.rol, email: p.email ?? f.email, telefono: p.telefono ?? f.telefono, linkedin: p.linkedin ?? f.linkedin }))}
        ...
        {[["Puesto en la empresa", "rol"], ["Email", "email"], ["Teléfono", "telefono"], ["LinkedIn", "linkedin"]].map(([lbl, key]) => (
          <div key={key}>
            ...
            <input value={(form as any)[key]} onChange={e => set(key, e.target.value)}
              className="w-full border border-gray-200 px-2 py-1.5 text-sm focus:outline-none focus:border-[#2E1A47]" />
          </div>
        ))}
        <div className="flex gap-2 pt-1">
          <button type="button" onClick={() => setEditing(false)} className="flex-1 py-1.5 text-xs font-semibold text-gray-600 border border-gray-200 hover:bg-gray-50">Cancelar</button>
          <button type="submit" disabled={saving} className="flex-1 py-1.5 text-xs font-bold text-white bg-[#2E1A47] hover:bg-[#3d2460] disabled:opacity-50">{saving ? "..." : "Guardar"}</button>
        </div>
      </form>
    );
  }
  ...
  return (
    <div className="py-3 flex items-start justify-between gap-3 group border-b border-gray-50 last:border-0">
      ...
      <div className="flex-shrink-0 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        {!confirm ? (
          ...
          <>
            <button onClick={handleDelete} disabled={deleting} className="text-[10px] font-bold text-white bg-red-500 px-2 py-0.5 hover:bg-red-600 disabled:opacity-50">{deleting ? "..." : "Sí"}</button>
            <button onClick={() => setConfirm(false)} className="text-[10px] text-gray-400 border border-gray-200 px-2 py-0.5">No</button>
          </>
        )}
      </div>
    </div>
...
function NuevoContactoForm({ clientId }: { clientId: string }) {
  const router = useRouter();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/admin/clientes/${clientId}/contactos`, {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form),
      });
      if (res.ok) { setForm({ nombre: "", rol: "", email: "", telefono: "", linkedin: "" }); setOpen(false); router.refresh(); }
    } finally { setSaving(false); }
  }

  if (!open) return (
    <button onClick={() => setOpen(true)} className="w-full mt-3 text-xs text-[#2E1A47] font-semibold border border-dashed border-[#2E1A47]/30 px-3 py-2 hover:bg-[#EEEBF3] transition-colors">+ Añadir contacto</button>
  );
...
export default function ContactosAdminPanel({ contactos, clientId }: { contactos: Contacto[]; clientId: string }) {
  return (
