...
import { db } from "@/db";
import { collaborators, entityContacts, financialEntities, entityContactNotes } from "@/db/schema";
import { eq } from "drizzle-orm";
import { notFound, redirect } from "next/navigation";
import Link from "next/link";
import NotesSection from "@/components/NotesSection";

export default async function PortalContactoEntidadPage({
  params,
}: {
  ...
  const [contacto] = await db.select().from(entityContacts).where(eq(entityContacts.id, contactoId)).limit(1);
  ...
  const notes = await db.select().from(entityContactNotes).where(eq(entityContactNotes.contact_id, contactoId)).orderBy(entityContactNotes.created_at);
    ...
    <div className="min-h-screen bg-[#f8f7fb]">
      ...
      <div className="mx-8 mb-6 bg-[#2E1A47] flex items-center gap-5 px-8 py-6">
        ...
        <div>
          ...
          <p className="text-white/40 text-xs mt-1">{entidad?.nombre}</p>
        </div>
      </div>
      ...
      <div className="mx-8 mb-6 grid grid-cols-3 gap-4 items-start">
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-white border border-gray-200 p-4">
            <p className="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Entidad financiera</p>
        ...
        <div className="col-span-2">
          <NotesSection
            notes={notes}
            apiUrl={`/api/admin/entidades/contactos/${contactoId}/notes`}
