...
import { db } from "@/db";
import { suppliers, operations, clients } from "@/db/schema";
...
const STATUS_LABEL: Record<string, string> = {
  pendiente_de_validar: "Pendiente", activa: "En curso", archivada: "Archivada",
};

export default async function PortalProveedorFichaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const ops = await db
    .select({
      id: operations.id, nombre: operations.nombre, pipeline_key: operations.pipeline_key,
      fase: operations.fase, status: operations.status, importe: operations.importe,
      created_at: operations.created_at, client_nombre: clients.nombre,
    })
    .from(operations).leftJoin(clients, eq(operations.client_id, clients.id))
    .where(and(eq(operations.supplier_id, id), eq(operations.collaborator_id, userId)))
  ...
  return (
    <div className="min-h-screen bg-[#f8f7fb]">
      <div className="px-8 pt-6 pb-2 flex items-center gap-2 text-xs text-gray-500">
        <Link href="/portal/proveedores" className="hover:text-[#2E1A47] font-medium">Mis proveedores</Link>
        <span>/</span>
      ...
      <div className="mx-8 mb-6 grid grid-cols-3 gap-4 items-start">
        <div className="flex flex-col gap-4">
          <div className="bg-white border border-gray-200">
            <div className="bg-[#EEEBF3] px-5 py-3 border-b border-gray-200"><h3 className="text-xs font-bold text-[#2E1A47] uppercase tracking-wider">Datos del proveedor</h3></div>
            <div className="px-5 py-4 divide-y divide-gray-50">
              {([["Email", prov.email], ["Teléfono", prov.telefono], ["Web", prov.web]] as [string, string | null][]).map(([label, value]) =>
                value ? (
            ...
            <div className="px-5 pb-4">
              <ProveedorEditFormPortal proveedor={{ id: prov.id, nombre: prov.nombre, email: prov.email ?? null, telefono: prov.telefono ?? null, web: prov.web ?? null, persona_contacto: prov.persona_contacto ?? null, contacto_email: prov.contacto_email ?? null, contacto_telefono: prov.contacto_telefono ?? null }} />
            </div>
          </div>

          {(prov.persona_contacto || prov.contacto_email || prov.contacto_telefono) && (
            <div className="bg-white border border-gray-200">
              <div className="bg-[#EEEBF3] px-5 py-3 border-b border-gray-200"><h3 className="text-xs font-bold text-[#2E1A47] uppercase tracking-wider">Persona de contacto</h3></div>
              <div className="px-5 py-4 divide-y divide-gray-50">
                {([["Nombre", prov.persona_contacto], ["Email", prov.contacto_email], ["Teléfono", prov.contacto_telefono]] as [string, string | null][]).map(([label, value]) =>
                  value ? (
                    <div key={label} className="py-2.5 flex flex-col gap-0.5">
                      <span className="text-[10px] text-gray-400 uppercase tracking-wide font-semibold">{label}</span>
                      <span className="text-sm text-gray-800 font-medium">{value}</span>
        ...
        <div className="col-span-2 bg-white border border-gray-200">
          <div className="bg-[#EEEBF3] px-5 py-3 border-b border-gray-200"><h3 className="text-xs font-bold text-[#2E1A47] uppercase tracking-wider">Operaciones asociadas</h3></div>
          {ops.length === 0 ? (
              ...
              <tbody className="divide-y divide-gray-50">
                {ops.map(op => (
                  <tr key={op.id} className="hover:bg-[#EEEBF3]/30 transition-colors">
                    ...
                    <td className="px-5 py-3"><span className={`inline-block px-2 py-0.5 text-xs font-semibold ${STATUS_BADGE[op.status] ?? ""}`}>{STATUS_LABEL[op.status] ?? op.status}</span></td>
                    <td className="px-5 py-3 text-sm font-medium text-gray-700 whitespace-nowrap">{fmtEur(op.importe)}</td>
                    <td className="px-5 py-3"><Link href={`/portal/operaciones/${op.id}`} className="text-xs text-[#2E1A47] font-semibold hover:underline">Ver →</Link></td>
                  </tr>
                ))}
              </tbody>
            </table>
