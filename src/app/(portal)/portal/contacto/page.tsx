    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#2E1A47] px-10 py-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <a href="https://www.linkedin.com/company/begreat-consulting" target="_blank" rel="noopener noreferrer"
          ...
          <a href="https://begreatconsulting.es" target="_blank" rel="noopener noreferrer"
      ...
      {/* ── Equipo + Documentación + Reunión en una fila ─────────────── */}
      <div className="grid grid-cols-3 gap-5">
            ...
            <div key={p.nombre} className="bg-white border border-gray-200 p-4 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-col gap-1 mt-2">
                  <a href={`tel:+34${p.telefono.replace(/\s/g, "")}`} className="text-xs text-gray-500 hover:text-[#2E1A47] transition-colors font-mono">{p.telefono}</a>
                  <a href={`mailto:${p.email}`} className="text-xs text-gray-400 hover:text-[#2E1A47] transition-colors truncate">{p.email}</a>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="col-span-1">
          ...
          <div className="bg-white border border-gray-200 p-5 space-y-3">
            <p className="text-xs text-gray-400 mb-4">Descarga nuestras presentaciones para compartir con tus clientes.</p>
            {docs.map((doc) => (
              <a key={doc.label} href={doc.href} download
        ...
        {/* Reunión */}
        <div className="col-span-1">
          <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Agenda</p>
          <div className="bg-[#2E1A47] p-5 text-white h-full flex flex-col justify-between" style={{ minHeight: 200 }}>
            <div>
              <p className="text-lg font-bold mb-2">Reserva una llamada</p>
              <p className="text-white/60 text-sm leading-relaxed">
                Accede a nuestro calendario y elige el horario que mejor te venga. Sin esperas.
              </p>
            </div>
            <a href="https://calendly.com/begreatconsulting" target="_blank" rel="noopener noreferrer"
      ...
      <div className="bg-white border border-gray-200 px-6 py-4 flex items-center justify-between">
        <div>
          <p className="text-xs text-gray-400 uppercase tracking-wider mb-0.5">Oficina principal</p>
          <p className="text-sm font-semibold text-gray-900">Serrano 118, Madrid</p>
        </div>
        <a href="https://maps.google.com/?q=Serrano+118+Madrid" target="_blank" rel="noopener noreferrer"
