...
export default async function ContactoDetallePage({ params }: { params: Promise<{ id: string; contactoId: string }> }) {
  ...
  // Verify client belongs to an operation of this supplier
  const [supplierOp] = await db.select({ id: operations.id })
    .from(operations).where(and(eq(operations.client_id, clientId), eq(operations.supplier_id, userId))).limit(1);
  ...
  return (
    <div className="min-h-screen bg-[#f8f7fb]">
      <div className="px-8 pt-6 pb-2 flex items-center gap-2 text-xs text-gray-500">
        ...
        <span>/</span>
        <Link href={`/proveedor/clientes/${clientId}`} className="hover:text-[#2E1A47] font-medium">{client.nombre}</Link>
        <span>/</span>
                ...
                if (label === "Empresa") {
                  return (
                    <div key={label} className="py-3 flex flex-col gap-0.5">
                      ...
                      <Link href={`/proveedor/clientes/${clientId}`} className="text-sm text-[#2E1A47] font-semibold hover:underline">{client.nombre} →</Link>
                    </div>
                  );
                }
