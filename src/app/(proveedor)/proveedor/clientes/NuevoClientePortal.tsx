...
export function NuevoClienteToggle() {
  ...
  return (
    <>
      ...
      {open && <NuevoClienteForm onClose={() => setOpen(false)} />}
    </>
  );
}
...
function NuevoClienteForm({ onClose }: { onClose: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/clientes", {
      ...
      body: JSON.stringify({
        nombre, cif, email, telefono, web, direccion, provincia, cnae,
    ...
    router.push(`/proveedor/clientes/${client.id}`);
  }
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input value={cif} onChange={e => setCif(e.target.value)}
              onBlur={e => { const v = formatDocId(e.target.value); setCif(v); }}
