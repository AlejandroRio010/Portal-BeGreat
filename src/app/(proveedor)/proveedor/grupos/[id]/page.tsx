...
export default async function ProveedorGrupoFichaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const myClientIds = [...new Set(myOps.map(o => o.client_id).filter(Boolean))] as string[];

  const empresas = myClientIds.length > 0
    ? await db
        .select({ id: clients.id, nombre: clients.nombre, codigo: clients.codigo, cif: clients.cif })
        .from(clients)
        .where(and(eq(clients.group_id, id), inArray(clients.id, myClientIds)))
  ...
  const ops = await db
    ...
    .where(and(inArray(operations.client_id, empresaIds), eq(operations.supplier_id, supplierId)))
  ...
  return (
    <div className="min-h-screen bg-[#f8f7fb]">
      <div className="px-8 pt-6 pb-2 flex items-center gap-2 text-xs text-gray-500">
        <Link href="/proveedor/grupos" className="hover:text-[#2E1A47] font-medium">Grupos empresariales</Link>
        <span>/</span>
        ...
        <div className="col-span-2">
          <div className="bg-white border border-gray-200">
            ...
            <div className="divide-y divide-gray-50">
              {empresas.map(e => (
                <Link key={e.id} href={`/proveedor/clientes/${e.id}`}
                  className="flex items-center justify-between px-5 py-3 hover:bg-[#EEEBF3]/30 transition-colors group">
                  <div>
                    <p className="text-sm font-semibold text-gray-800 group-hover:text-[#2E1A47]">{e.nombre}</p>
                    <p className="text-[10px] text-gray-400">{[e.codigo, e.cif].filter(Boolean).join(" · ")}</p>
                  </div>
                  <span className="text-xs text-[#2E1A47] opacity-0 group-hover:opacity-100">Ver →</span>
                </Link>
              ))}
            </div>
          </div>
      ...
      <div className="mx-8 mb-8">
        <div className="bg-white border border-gray-200 overflow-hidden">
          ...
          {ops.length === 0 ? (
            <p className="px-5 py-8 text-sm text-gray-400 text-center">Sin operaciones registradas.</p>
          ) : (
            <div style={{ zoom: 0.9 }}>
                    ...
                    return (
                      <tr key={op.id} className="hover:bg-[#EEEBF3]/30 transition-colors group">
                        ...
                        <td className="px-5 py-3 text-right whitespace-nowrap">
                          <Link href={`/proveedor/operaciones/${op.id}`} className="text-xs text-[#2E1A47] font-semibold opacity-0 group-hover:opacity-100 transition-opacity hover:underline">Ver →</Link>
                        </td>
                      </tr>
                    );
                  })}
