...
export default async function ProveedorLayout({ children }: { children: React.ReactNode }) {
  ...
  const [supplier] = await db
    .select({ nombre: suppliers.nombre, codigo: suppliers.codigo, puede_ver_entidades: suppliers.puede_ver_entidades })
    .from(suppliers)
  ...
  return (
    <div className="flex min-h-screen">
      <Sidebar nombre={nombre} identificador={identificador} role="proveedor" puedeVerEntidades={supplier?.puede_ver_entidades ?? false} />
